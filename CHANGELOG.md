...
- 관리자는 카테고리와 쉬운 업종명·예시로 표준 업종을 찾고, 상품·서비스 이름, 기존 이름, MCC 코드, 동의어로 전체 목록을 검색해 여러 업종을 한 번에 등록할 수 있어요. 선택한 업종은 카테고리와 검색을 바꿔도 유지되고, 이미 등록된 업종은 선택에서 제외돼요.
- 관리자는 등록된 업종도 같은 카테고리와 검색으로 찾아 설정을 수정할 수 있어요. MCC 코드와 판본은 분류 정보를 펼쳐 관리할 때 확인해요.
...
- 운영자가 PG사별 장기합의서 표준 수수료를 결제수단·가맹점 등급별로 등록하고 변경할 수 있어요. 빈칸은 미등록, 0은 명시적인 0으로 구분해요.
- 다른 관리자가 먼저 기준을 바꾸면 오래 열린 화면의 저장을 막고, 변경 전후 기준을 감사 기록에 남겨요.
- 구매사가 입력한 결제수단 이름에 공백이나 줄바꿈이 있어도 원문 그대로 기준을 등록할 수 있고, 다른 항목을 수정할 때 기존 요율의 정밀도를 유지해요.

## [0.4.0.0] - 2026-09-20

### Added

- 운영자가 업종별 입점 위험도를 일반·추가 검토·접수 불가로 설정하고, 상담을 제안할 PG사의 순서와 안내 사유를 관리할 수 있어요.
- PG사별 제안 수수료 범위와 안내 문구를 설정하면 구매사 추천 화면에 반영돼요. 접수 불가 업종은 상담 요청을 받지 않아요.
...
- RFP 상세 페이지에서 견적서 세부사항을 확인할 수 있습니다 — 구매사가 제출한 현재조건(수수료율·정산조건·솔루션 등), 요청 결제수단, RFP 메모·첨부파일을 표시하고, PG별 견적을 결제수단×견적 수수료 비교표로 한눈에 볼 수 있습니다(구간별 우대수수료·정액 결제수단·커스텀 결제수단 모두 지원, 같은 PG의 재견적도 별도 열로 구분). 견적별 메모, 제안서 첨부, 낙찰/계약 정보도 함께 표시됩니다.
- RFP 상세 정보에 사업자번호·과세유형·영중소구간(확정 방식 포함)이 추가로 표시됩니다.

### Fixed

- 구매사가 입력한 웹사이트 URL을 관리자 화면에서 클릭할 때, 잘못된 스킴(`javascript:` 등)이 실행되지 않도록 http/https만 링크로 렌더하도록 수정했습니다.
- 결제수단별 수수료가 비어있거나 잘못된 형태로 저장된 경우에도 화면이 오류 없이 정상적으로 표시되도록 방어 코드를 추가했습니다.
- `pnpm lint`가 TypeScript 관련 규칙 없이 통과되던 설정 오류를 수정했습니다.
