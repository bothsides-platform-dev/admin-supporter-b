...
describe('workspace name change review', () => {
  it('승인하면 요청과 워크스페이스 이름을 한 트랜잭션에서 반영하고 감사를 남긴다', async () => {
    ...
    expect(await approveWorkspaceNameChangeAction(db, requestId)).toEqual({ ok: true });
  ...
  it('거절하면 현재 이름을 유지하고 사유를 기록한다', async () => {
    const { workspaceId, requestId } = await seed();
    expect(await rejectWorkspaceNameChangeAction(db, requestId, '  증빙이 필요합니다.  ')).toEqual({ ok: true });
  ...
  it('이미 처리된 요청은 다시 처리하지 않는다', async () => {
    ...
    expect(await approveWorkspaceNameChangeAction(db, requestId)).toEqual({ ok: true });
    await expect(approveWorkspaceNameChangeAction(db, requestId)).resolves.toEqual({ ok: false, error: 'REQUEST_NOT_PENDING' });
    await expect(rejectWorkspaceNameChangeAction(db, requestId, '뒤늦은 거절')).resolves.toEqual({ ok: false, error: 'REQUEST_NOT_PENDING' });
  });
  ...
  it('요청 뒤 워크스페이스가 정지되면 승인하지 않는다', async () => {
    ...
    await expect(approveWorkspaceNameChangeAction(db, requestId)).resolves.toEqual({ ok: false, error: 'WORKSPACE_NOT_ACTIVE' });
    expect((await client.query<{ name: string }>('SELECT name FROM workspaces WHERE id=$1', [workspaceId])).rows[0].name).toBe('기존 이름');
  ...
  it('존재하지 않는 요청과 빈 거절 사유를 거부한다', async () => {
    ...
    await expect(approveWorkspaceNameChangeAction(db, missingId)).resolves.toEqual({ ok: false, error: 'REQUEST_NOT_PENDING' });
    await expect(rejectWorkspaceNameChangeAction(db, missingId, '거절 사유')).resolves.toEqual({ ok: false, error: 'REQUEST_NOT_PENDING' });
    await expect(rejectWorkspaceNameChangeAction(db, missingId, '   ')).resolves.toEqual({ ok: false, error: 'INVALID_INPUT' });
  });

  it('UUID가 아닌 요청 id와 500자를 넘는 거절 사유를 서버에서 거부한다', async () => {
    await expect(approveWorkspaceNameChangeAction(db, 'not-a-uuid')).resolves.toEqual({ ok: false, error: 'INVALID_INPUT' });
    await expect(rejectWorkspaceNameChangeAction(
      db,
  ...
  it('워크스페이스가 삭제된 요청도 거절하고 이력과 목록 캐시를 갱신한다', async () => {
    ...
    expect(await rejectWorkspaceNameChangeAction(db, requestId, '회사를 확인할 수 없습니다.')).toEqual({ ok: true });
  ...
  it('CAS 경합에서 요청 claim을 잃으면 후속 쓰기를 하지 않는다', async () => {
    ...
    await expect(approveWorkspaceNameChangeAction(fakeDb, requestId)).resolves.toEqual({ ok: false, error: 'REQUEST_NOT_PENDING' });
