...
describe('listPgRecommendationGroups', () => {
  it('업종마다 소속 PG를 반환하고 이름순으로 정렬한다', async () => {
    ...
    await client.exec(`
      CREATE TABLE pg_recommendation_groups (id uuid PRIMARY KEY, name text NOT NULL, sort_order integer NOT NULL, mcc_code text, mcc_version text);
      ...
      INSERT INTO pg_recommendation_groups VALUES
        ('10000000-0000-4000-8000-000000000001', '여행', 2, '4722', 'visa-2026-04'),
        ('10000000-0000-4000-8000-000000000002', '쇼핑', 1, null, null);
    ...
    expect(await listPgRecommendationGroups(drizzle(client))).toEqual([
      {
        ...
        name: '쇼핑', mccCode: null, mccVersion: null,
        sortOrder: 1,
        pgWorkspaceIds: [
      ...
      },
      {
        ...
        name: '여행', mccCode: '4722', mccVersion: 'visa-2026-04',
        sortOrder: 2,
        pgWorkspaceIds: [],
      },
    ]);
