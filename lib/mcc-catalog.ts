...
export function searchMccIndustries(query: string) {
  const terms = query.trim().toLowerCase().split(/\s+/).filter(Boolean);
  return MCC_INDUSTRIES.filter(item => terms.every(term => `${item.code} ${item.name} ${item.category}`.toLowerCase().includes(term)));
}
